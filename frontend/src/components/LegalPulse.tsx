    ...
    <section>
      ...
      <div className="rounded-2xl border border-white/[0.06] bg-[#0d0d1a] overflow-hidden divide-y divide-white/[0.05]">
        {NEWS_ITEMS.map((item, i) => (
          <div
            key={i}
            className="group flex items-start gap-3.5 px-4 py-3.5 cursor-pointer hover:bg-white/[0.025] transition-colors"
          >
            {/* Gold left bar */}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[11px] text-white/90 font-medium leading-snug line-clamp-2 group-hover:text-white transition-colors">
                {item.headline}
              </p>
              <div className="mt-1.5 flex items-center gap-2">
                <span className="text-[10px] text-white/30">{item.date}</span>
                <span className="rounded-full bg-white/[0.07] px-1.5 py-0.5 text-[9px] font-medium text-white/40">
                  {item.source}
                </span>
              </div>
            </div>

            <ArrowRight className="h-3.5 w-3.5 shrink-0 text-white/15 group-hover:text-white/40 transition-colors mt-1" />
          </div>
        ))}
      </div>
    </section>
